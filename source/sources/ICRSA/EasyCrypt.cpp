...
#include "headers/ICRSA/KeyPair.h"
#include "headers/ICRSA/Key.h"
#include "headers/ICRSA/BigInt.h"
 ...
 */
int AESBEncrypt(u32 *rk, const u8 *key, int keybits)
{
  int i = 0;
  u32 temp;
 ...
 * @return the number of rounds for the given cipher key size.
 */
int AESBDecrypt(u32 *rk, const u8 *key, int keybits)
{
  int nrounds, i, j;
  u32 temp;

  /* expand the cipher key: */
  nrounds = AESBEncrypt(rk, key, keybits);
...
void AESEncrypt(const u32 *rk, int nrounds, const u8 plaintext[16],
  u8 ciphertext[16])
{
...
void AESDecrypt(const u32 *rk, int nrounds, const u8 ciphertext[16],
  u8 plaintext[16])
{
  u32 s0, s1, s2, s3, t0, t1, t2, t3;
  #ifndef FULL_UNROLL
...
//Fonction visible par l'utilisateur
std::string ec::AES::Encrypt(std::string plaintext, std::string key, int keybits)
{
    std::string keyd;
...
std::string ec::AES::Decrypt(std::string text, std::string key, int keybits)
{
	...
	if(key[0] != 0 && key[1] != 0 && key[2] != 0){
        ...
        if(finale[0] != (unsigned char)NULL)
			str += finale[0];
        if(finale[1] != (unsigned char) NULL)
			str += finale[1];
        if(finale[2] != (unsigned char) NULL)
			str += finale[2];
        if(finale[3] !=  (unsigned char)NULL)
			str += finale[3];
        if(finale[4] !=  (unsigned char)NULL)
			str += finale[4];
        if(finale[5] !=  (unsigned char)NULL)
			str += finale[5];
        if(finale[6] !=  (unsigned char)NULL)
			str += finale[6];
        if(finale[7] !=  (unsigned char)NULL)
			str += finale[7];
        if(finale[8] !=  (unsigned char)NULL)
			str += finale[8];
        if(finale[9] !=  (unsigned char)NULL)
			str += finale[9];
        if(finale[10] !=  (unsigned char)NULL)
			str += finale[10];
        if(finale[11] !=  (unsigned char)NULL)
			str += finale[11];
        if(finale[12] !=  (unsigned char)NULL)
			str += finale[12];
        if(finale[13] !=  (unsigned char)NULL)
			str += finale[13];
        if(finale[14] !=  (unsigned char)NULL)
			str += finale[14];
        if(finale[15] !=  (unsigned char)NULL)
			str += finale[15];
...
std::string ec::AES::GenerateKey(int keybits)
{
	std::srand(time(NULL));
	std::string key;
	int i = 0;
