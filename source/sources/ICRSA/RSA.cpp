...
#include "headers/ICRSA/Key.h"	//Key
#include "headers/ICRSA/KeyPair.h"	//KeyPair
#include "headers/ICRSA/PrimeGenerator.h"	//Generate()
...
BigInt RSAH::GCD(const BigInt &a, const BigInt &b)
{
	if (b.EqualsZero())
		return a;
	else
...
void RSAH::extendedEuclideanAlgorithm(const BigInt &a, const BigInt &b, BigInt &d, BigInt &x, BigInt &y)
{
	if (b.EqualsZero())
	{
		d = a;
	...
	extendedEuclideanAlgorithm(b, a % b, d, x, y);
	BigInt temp(x);
	x = y;
	y = temp - a / b * y;
}
...
BigInt RSAH::solveModularLinearEquation(const BigInt &a, const BigInt &b, const BigInt &n)
{
	BigInt p, q, r;
	extendedEuclideanAlgorithm(a, n, p, q, r);
...
void RSAH::checkKeyLength(const Key &key)
{
...
BigInt RSAH::encode(const string &message)
{
...
string RSAH::decode(const BigInt &message)
{
	string decoded;
...
string RSAH::encryptChunk(const string &chunk, const Key &key)
{
	// First encode the chunk, to make sure it is represented as an integer.
	BigInt a = encode(chunk);
...
string RSAH::decryptChunk(const BigInt &chunk, const Key &key)
{
	BigInt a = chunk;
...
std::string RSAH::encryptString(const std::string &message, const Key &key)
{
	...
	lastChunk = encryptChunk(lastChunk, key);
	return cypherText.append(lastChunk.append(" "));
}

/* Decrypts a string "message" using "key". */
std::string RSAH::decryptString(const std::string &cypherText, const Key &key)
{
	...
	while ((j = cypherText.find(' ', i)) != -1)
	{
		...
		string text = decryptChunk(chunk, key);
		message.append(text);
		i = j + 1;
	}
	return message;
...
void RSAH::fileError(bool eof, bool bad)
{
	if (eof)
...
string RSAH::Encrypt(const string &message, const Key &key)
{
	checkKeyLength(key);
...
void RSAH::Encrypt(const char *sourceFile, const char *destFile, const Key &key)
{
	checkKeyLength(key);
	...
	for (unsigned long int i(0); i <= chunkCount; i++)
	{
		...
		if (!source)
			fileError(source.eof(), source.bad());

		//encrypt the chunk
		std::string chunk(buffer, readLength);
		chunk = encryptString(chunk, key);
		//write the chunk
		dest.write(chunk.c_str(), chunk.length());
		if (!dest)
			fileError(dest.eof(), dest.bad());
	}
...
string RSAH::Decrypt(const string &cypherText, const Key &key)
{
	checkKeyLength(key);

	return decryptString(cypherText, key);
}

/* Decrypts the file "sourceFile" using the key "key" and saves
 * the result into the file "destFile". */
void RSAH::Decrypt(const char *sourceFile, const char *destFile, const Key &key)
{
	checkKeyLength(key);
...
KeyPair RSAH::GenerateKeyPair(unsigned long int digitCount, unsigned long int k)
{
	if (digitCount < 8)
	...
	BigInt d(solveModularLinearEquation(e, BigIntOne, phi));

	//we need a positive private exponent
	if (!d.IsPositive())
		return GenerateKeyPair(digitCount, k);
