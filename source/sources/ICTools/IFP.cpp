...
void IChemSwitch::IFP() const throw(MoleExcept)
{
    ...
    if (!Opt_Values.empty())
              ...
              ++ItOption)
        {
            ...
            else if (key.compare("--properties") == 0) {
                // globalConfigOptions.displayProperties = true;
...
//        }
        ...
        // The first part ## Protein - Ligand - LigandRef ##
        if (w_ref){
...
//                intsosef.calcInteractions(*icomplex.getMole(MoleType::PROTEIN),osef,false,oldh,ligd);
        ...
        // The second part ## Protein - Ligand ##
        } else {
            // 1.2 - Loading Ligand :
            iread.loadNewFile(fLigand);
            const size_t nLigand=iread.getNumMolecules();
            ...
            //    intsosef.calcInteractions(*icomplex.getMole(MoleType::PROTEIN),osef,false,oldh,ligd);
                ints.calcInteractions(ligand, interRes, true, oldh, ligd, displayProperties);
            ...
            else
            {
                ...
                while (!iread.isEOF())
                {
                    ...
                    iread.loadNextMolecule(ligand,MoleType::LIGAND);
                    names.push_back(ligand.getName());
                   // ringPerception() calls AddCycle method that detects the aromatic cycl
                   // Adds it to the cycles vector of the molecule
                   ligand.checkMOL2();
                   ligand.ringPerception();
                    Interactions intsosef(icomplex);
                    if (changedist){
                   ...
                   InterResults osef;
                   intsosef.calcInteractions(*icomplex.getMole(MoleType::PROTEIN), osef, false, oldh, ligd, displayProperties);

                   ints.calcInteractions(ligand, interRes[currLig],true, oldh, ligd, displayProperties);
