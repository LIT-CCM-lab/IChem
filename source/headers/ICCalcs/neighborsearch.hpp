...
public:
    ...
    // We build KDTree from the set of points
    ...
    // For perf purposes we change the std::pair into the struct and we avoid also the sqrt function
    struct Contact {
        int ligand_idx;
        int protein_idx;
    ...
    // The Query neighbors is within the interval distance [min, max]
    // Return pair (i,j) 
    std::vector<Contact> query(const std::vector<std::array<double, 3>>& queryPoints, double dist_max, double dist_min = 0.0) const {
